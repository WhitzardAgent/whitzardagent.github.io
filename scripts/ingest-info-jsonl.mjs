...
const researchAssets = researchLines.map((line, idx) => {
  ...
  // Topic: semicolon-separated string → array
  let topic = undefined;
  const rawTopic = raw.Topic || raw.topic || raw.Tags || raw.tags;
  if (rawTopic) {
  ...
  const summary =
    raw["One-line summary"] || raw.summary || raw.description || undefined;
...
const openSourceAssets = osLines
  .map((line, idx) => {
    ...
    const description =
      raw["One-line description"] || raw.description || undefined;
