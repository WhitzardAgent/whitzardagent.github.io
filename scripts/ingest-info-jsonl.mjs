...
const researchInfrastructureFile = join(infoDir, "research_infrastructure_metadata.jsonl");
const researchInfrastructureLines = readFileSync(researchInfrastructureFile, "utf-8")
  .trim()
  .split("\n")
  .filter(Boolean);

const researchInfrastructure = researchInfrastructureLines.map((line, idx) => {
  const raw = JSON.parse(line);
  const name = String(raw.Name || raw.name || `Research infrastructure ${idx + 1}`);
  const typeEn = String(raw.Type || raw.type || "Research infrastructure");
  const typeZh = String(raw["Type zh"] || raw.typeZh || typeEn);
  const descriptionEn = String(raw["One-line description"] || raw.description || "");
...
if (new Set(researchInfrastructure.map((item) => item.slug)).size !== researchInfrastructure.length) {
  throw new Error("Duplicate research infrastructure record");
}

const researchInfrastructureTs = `// Auto-generated by scripts/ingest-info-jsonl.mjs
// DO NOT EDIT MANUALLY — re-run npm run ingest:info to update
...
export const researchInfrastructure: ResearchInfrastructure[] = ${JSON.stringify(researchInfrastructure, null, 2)} as const;
`;

writeFileSync(join(outDir, "researchInfrastructure.ts"), researchInfrastructureTs);
console.log(`Generated researchInfrastructure.ts with ${researchInfrastructure.length} entries`);
