...
export const agentGuardSystemNodes: SystemNode[] = [
  ...
  { id: "tools", kind: "tool", shortLabel: t("工具", "Tools"), label: t("工具、Shell 与浏览器", "Tools, shell, and browser"), handling: t("读取资源、执行命令并改变系统状态", "Reads resources, executes commands, and changes system state"), challenge: t("单步合法动作组合后可能形成高风险链路", "Individually valid actions can combine into a dangerous chain"), tracking: t("能力、参数、目标环境与行动影响", "Capability, arguments, target environment, and impact"), response: t("在调用前后执行限权、降级、沙箱或阻断", "Limit, degrade, sandbox, or deny around invocation boundaries"), position: { x: 15, y: 75 } },
...
export const agentGuardStoryScenarios: StoryScenario[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "commit-shell", label: t("验证 Shell 命令", "Validate shell command"), system: t("Shell", "Shell"), flowKinds: ["authorization", "effect"], state: t("无生产执行权，行动影响为 EXECUTE", "No production execution grant; impact is EXECUTE"), intervention: "SWITCH_TO_SANDBOX", explanation: t("命令只在隔离环境生成预览，不改变生产状态", "Run only as an isolated preview without changing production state"), before: t("直接执行修复脚本", "Execute remediation script directly"), after: t("沙箱预览 · 变更差异 · 无副作用", "Sandbox preview · Change diff · No side effects"), technical: t("Tool Before · impact downgrade", "Tool Before · impact downgrade") },
...
export const storyPageCopy = {
  zh: {
    map: { eyebrow: "运行时架构", title: "智能体执行与控制拓扑", body: "从任务与身份到外部行动，AgentGuard 沿完整执行路径持续追踪数据、授权与行动影响", runtime: "AgentGuard 交互边界运行时", live: "实时保护中", select: "选择保护对象", incoming: "进入节点", outgoing: "离开节点", impact: "潜在影响路径", handling: "当前处理", challenge: "关键难点", tracking: "AgentGuard 追踪", response: "AgentGuard 处理", pause: "暂停", replay: "重播", protected: "保护对象" },
    engine: { eyebrow: "统一安全影响引擎", title: "细粒度管控行为链、思维链和数据链", body: "分别理解行动组合、任务意图与数据传播，并在每个交互边界联合判断", tracks: "追踪对象", distinction: "为何独立分析", contribution: "如何形成决策", core: "AgentGuard Unified Security Influence Engine", result: "最小必要处置", evidence: "查看技术依据", evidenceBody: "任务目标、传播血缘、目标边界与策略约束在同一边界状态中求值" },
  ...
  en: {
    map: { eyebrow: "RUNTIME ARCHITECTURE", title: "Agent Execution and Control Topology", body: "From task identity to external action, AgentGuard tracks data, authorization, and action impact across the complete execution path", runtime: "AgentGuard Interaction Boundary Runtime", live: "Protecting in real time", select: "Select a protected object", incoming: "Incoming", outgoing: "Outgoing", impact: "Potential impact path", handling: "Current role", challenge: "Security challenge", tracking: "AgentGuard tracks", response: "AgentGuard response", pause: "Pause", replay: "Replay", protected: "Protected objects" },
    engine: { eyebrow: "UNIFIED SECURITY INFLUENCE ENGINE", title: "Control behavior, reasoning, and data chains", body: "Understand action composition, task intent, and data propagation together at every interaction boundary", tracks: "What it tracks", distinction: "Why it differs", contribution: "How it shapes the decision", core: "AgentGuard Unified Security Influence Engine", result: "Minimum necessary control", evidence: "View technical evidence", evidenceBody: "Task intent, propagation lineage, target boundary, and policy constraints are evaluated in one boundary state" },
