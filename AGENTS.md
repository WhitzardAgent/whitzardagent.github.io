...
- Follow the bilingual route contract: Chinese is the default locale and unprefixed; English lives under `/en/*`. Legacy `/zh/*` URLs are preserved as static redirect stubs in `public/zh/`. Keep locale metadata, design tokens, WCAG AA contrast, responsive behavior, and `prefers-reduced-motion` support intact.
